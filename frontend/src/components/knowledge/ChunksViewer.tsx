...
export const ChunksViewer: React.FC<ChunksViewerProps> = ({
  ...
  const [vectorLookupError, setVectorLookupError] = useState<string | null>(null);
  const pageSize = 20;
  const maxRetries = 20;
    ...
    <div>
      ...
      {vectorLookupError && (
        <div className="mb-3 text-xs text-amber-700 dark:text-amber-300 bg-amber-500/10 rounded-md px-3 py-2">
          Vector index status lookup failed: {vectorLookupError}
        </div>
      )}
      ...
      <div className="space-y-2">
        {chunks.map((chunk) => {
          ...
          const embeddingStatus: EmbeddingStatus =
            (chunk.embedding?.status as EmbeddingStatus | undefined) || 'unknown';
          const embeddingStatusLabel = embeddingStatus;
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-3">
                      ...
                      <p className="text-sm text-gray-700 dark:text-gray-300">
                        {enrichmentApplied ? 'Applied' : 'Not applied'}
                      </p>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        keywords: {chunk.enrichment?.keywords_count ?? chunk.keywords?.length ?? 0}
                        {'  '}| questions:{' '}
                        {chunk.enrichment?.questions_count ?? chunk.questions?.length ?? 0}
                        {'  '}| summary:{' '}
                        {(chunk.enrichment?.has_summary ?? Boolean((chunk.summary || '').trim()))
                          ? 'yes'
                          : 'no'}
                      </p>
                    </div>
                    <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-3">
                      ...
                      <p className="text-sm text-gray-700 dark:text-gray-300">{embeddingStatus}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        {chunk.embedding?.provider || embeddingConfig?.provider || '-'} /{' '}
                        {chunk.embedding?.model || embeddingConfig?.model || '-'}
                        {chunk.embedding?.dimension || embeddingConfig?.dimension
