...
class MinIOClient:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        logger.info(
            f"MinIO client initialized for endpoint: {self.config['endpoint']}"
        )
    ...
    def validate_file_type(self, filename: str, bucket_type: str) -> bool:
        ...
        if not allowed:
            logger.warning(
                f"File type '{file_ext}' not allowed for bucket type '{bucket_type}'"
            )
    ...
    def upload_file(
        ...
        try:
            ...
            logger.info(
                f"File uploaded successfully: {bucket_name}/{object_key} "
                f"({file_size} bytes)"
            )
    ...
    def list_objects(
        ...
        try:
            ...
            logger.info(
                f"Listed {len(result)} objects from {bucket_name} "
                f"with prefix '{prefix or ''}'"
            )
        ...
        try:
            ...
            for obj in objects:
                if obj.last_modified and obj.last_modified < cutoff_date:
                    ...
                    deleted_count += 1
                    logger.debug(
                        f"Deleted temporary file: {bucket_name}/{obj.object_name}"
                    )
    ...
    def copy_object(
        ...
        try:
            ...
            logger.info(
                f"Object copied: {source_bucket}/{source_key} -> "
