...
class FileMetadata(Base):
    ...
    transcription_status = Column(String(50), nullable=True)  # pending, completed, failed, not_applicable
...
class FileMetadataManager:
    ...
    def create_metadata(
        ...
        logger.info(
            f"Created file metadata: {file_meta.file_id} "
    ...
    def list_by_agent(self, agent_id: UUID) -> List[FileMetadata]:
        ...
        """
        return self.db.query(FileMetadata).filter(
            FileMetadata.agent_id == agent_id,
            FileMetadata.is_deleted == False
        ).order_by(FileMetadata.created_at.desc()).all()
