...
class TaskDecomposer:
    ...
    async def decompose_goal(
        ...
        total_duration = sum(
            task.estimated_duration or 30 for task in all_tasks
        )
...
{{
    ...
    def _is_atomic_task(self, goal_text: str) -> bool:
        ...
        atomic_verbs = [
            "fetch", "retrieve", "read", "write", "save",
            "send", "receive", "calculate", "format", "validate"
        ]
        
        has_atomic_verb = any(
            verb in goal_text.lower() for verb in atomic_verbs
        )
    ...
    def _get_max_depth(self, root_task: DecomposedTask) -> int:
        ...
        return 1 + max(
            self._get_max_depth(subtask) for subtask in root_task.subtasks
        )
