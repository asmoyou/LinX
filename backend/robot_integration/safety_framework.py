...
from robot_integration.world_state import WorldState, RobotPose
...
class SafetyChecker:
        ...
        if task.constraints and task.constraints.max_speed_ms:
            if task.constraints.max_speed_ms > max_speed:
                return SafetyViolation(
                    ...
                    safety_level=rule.violation_level,
                    context={"requested_speed": task.constraints.max_speed_ms, "max_speed": max_speed},
                )
        ...
        if task.constraints and task.constraints.max_force_n:
            if task.constraints.max_force_n > max_force:
                return SafetyViolation(
                    ...
                    safety_level=rule.violation_level,
                    context={"requested_force": task.constraints.max_force_n, "max_force": max_force},
                )
    ...
    def check_emergency_stop_required(
        ...
        for violation in self.violations:
            if (violation.robot_id == robot_id and 
                not violation.resolved and
                violation.safety_level == SafetyLevel.EMERGENCY):
