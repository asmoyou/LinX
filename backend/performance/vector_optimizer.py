...
class VectorSearchOptimizer:
    ...
    def get_optimization_recommendations(
        ...
        if current_latency_ms > target_latency_ms * 2:
            ...
            recommendations.append(
                "Reduce nprobe parameter to search fewer clusters"
            )
            recommendations.append(
                "Enable result caching for frequently searched queries"
            )
        
        if current_qps > 100:
            recommendations.append(
                "Implement query batching to improve throughput"
            )
            recommendations.append(
                "Consider horizontal scaling with multiple Milvus instances"
            )
        
        index_config = self.index_configs.get(collection_name)
        if index_config and index_config.index_type == IndexType.FLAT:
            recommendations.append(
                "Upgrade from FLAT to IVF_FLAT or HNSW for better performance"
            )
