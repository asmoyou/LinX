...
async def test_ollama_generate_success():
    ...
    with patch.object(provider, "_get_session", side_effect=mock_get_session):
        response = await provider.generate(prompt="Test prompt", model="llama3", temperature=0.7)
...
async def test_ollama_generate_embedding():
    ...
    mock_response = {"embedding": [0.1, 0.2, 0.3], "prompt_eval_count": 5, "total_duration": 500000}
    ...
    with patch.object(provider, "_get_session", side_effect=mock_get_session):
        response = await provider.generate_embedding(text="Test text", model="nomic-embed-text")
...
async def test_vllm_provider_initialization():
    ...
    config = {"base_url": "http://localhost:8000", "timeout": 120, "api_key": "test-key"}
