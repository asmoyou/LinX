...
from skill_library.skill_registry import SkillRegistry, SkillInfo, get_skill_registry
...
class SemanticSkillSearch:
    ...
    def find_similar_skills(
        ...
        for skill in all_skills:
            ...
            skill_embedding = self.embedding_service.generate_embedding(
                skill.description
            )
    ...
    def suggest_existing_skill(
        ...
        if similar_skills:
            skill, similarity = similar_skills[0]
            logger.info(
                f"Suggesting existing skill: {skill.name} "
