...
class ConfigValidator:
    ...
    def _validate_required_sections(self) -> List[ValidationResult]:
        ...
        for section in required_sections:
            if not self.config.has(section):
                results.append(
                    ValidationResult(False, f"Required section '{section}' is missing", section)
                )
            else:
        ...
        if exp_hours is not None:
            results.append(
                self._validate_range(
                    "api.jwt.expiration_hours", exp_hours, min_val=1, max_val=720  # 30 days
                )
            )
    ...
    def _validate_database(self) -> List[ValidationResult]:
        ...
        if pool_size is not None:
            results.append(
                self._validate_range(
                    "database.postgres.pool_size", pool_size, min_val=1, max_val=100
                )
            )
            ...
            for provider_name, provider_config in providers.items():
                if provider_config.get("enabled", False):
                    ...
                    if not models:
                        ...
                        results.append(
                            ValidationResult(True, "", f"llm.providers.{provider_name}.models")
                        )
    ...
    def _validate_agents(self) -> List[ValidationResult]:
        ...
        # Resource limits
        results.append(
            self._validate_required_field("agents.resources.default_cpu_cores", (int, float))
        )
        results.append(
            self._validate_required_field("agents.resources.default_memory_gb", (int, float))
        )
        results.append(
            self._validate_required_field("agents.resources.max_cpu_cores", (int, float))
        )
        results.append(
            self._validate_required_field("agents.resources.max_memory_gb", (int, float))
        )
    ...
    def _validate_security(self) -> List[ValidationResult]:
        ...
        if self.config.get("security.data_classification.enabled", False):
            results.append(
                self._validate_required_field("security.data_classification.levels", list)
            )
    ...
    def _validate_dependencies(self) -> List[ValidationResult]:
        ...
        if self.config.get("security.encryption.at_rest", False):
            ...
            results.append(
                ValidationResult(
                    True, "Encryption at rest is enabled", "security.encryption.at_rest"
                )
            )
        ...
        if self.config.get("security.encryption.in_transit", False):
            results.append(
                ValidationResult(
                    True, "Encryption in transit is enabled", "security.encryption.in_transit"
                )
            )
    ...
    def _validate_required_field(
        self, field: str, expected_type: Union[type, Tuple[type, ...]]
    ) -> ValidationResult:
        """
        ...
        if value is None:
            return ValidationResult(False, f"Required field is missing", field)
        ...
        if not isinstance(value, expected_type):
            ...
            return ValidationResult(
                False, f"Expected type {type_names}, got {type(value).__name__}", field
            )
    ...
    def _validate_port(self, field: str, port: int) -> ValidationResult:
        ...
        if not isinstance(port, int):
            return ValidationResult(
                False, f"Port must be an integer, got {type(port).__name__}", field
            )

        if port < 1 or port > 65535:
            return ValidationResult(False, f"Port must be between 1 and 65535, got {port}", field)
    ...
    def _validate_range(
        ...
        if not isinstance(value, (int, float)):
            return ValidationResult(
                False, f"Value must be numeric, got {type(value).__name__}", field
            )

        if min_val is not None and value < min_val:
            return ValidationResult(False, f"Value must be >= {min_val}, got {value}", field)

        if max_val is not None and value > max_val:
            return ValidationResult(False, f"Value must be <= {max_val}, got {value}", field)
    ...
    def _validate_url(self, field: str, url: str) -> ValidationResult:
        ...
        if not isinstance(url, str):
            return ValidationResult(False, f"URL must be a string, got {type(url).__name__}", field)
        ...
        if not url_pattern.match(url):
            return ValidationResult(False, f"Invalid URL format: {url}", field)
    ...
    def _validate_path(self, field: str, path: str, must_exist: bool = False) -> ValidationResult:
        ...
        if not isinstance(path, str):
            return ValidationResult(
                False, f"Path must be a string, got {type(path).__name__}", field
            )

        if must_exist:
            path_obj = Path(path)
            if not path_obj.exists():
                return ValidationResult(False, f"Path does not exist: {path}", field)
