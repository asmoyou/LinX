...
from database.models import Agent, ResourceQuota, User
...
class ResourceQuotaManager:
    ...
    def get_quota_usage(self, user_id: UUID) -> QuotaUsage:
        ...
        with get_db_session() as session:
            quota = session.query(ResourceQuota).filter(ResourceQuota.user_id == user_id).first()
    ...
    def remove_storage_usage(self, user_id: UUID, size_bytes: int) -> None:
        ...
        with get_db_session() as session:
            ...
            if quota:
                quota.current_storage_gb = max(Decimal("0"), quota.current_storage_gb - size_gb)
                session.commit()
    ...
    def create_default_quota(self, user_id: UUID) -> ResourceQuota:
        ...
        with get_db_session() as session:
            ...
            existing = session.query(ResourceQuota).filter(ResourceQuota.user_id == user_id).first()
    ...
    def get_all_quotas_summary(self) -> Dict[str, Any]:
        ...
        with get_db_session() as session:
            ...
            agents_near_limit = sum(
                1 for q in quotas if q.max_agents > 0 and (q.current_agents / q.max_agents) >= 0.8
            )
