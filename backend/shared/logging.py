    ...
    if log_output in ["file", "both"]:
        ...
        if log_rotation == "daily":
            file_handler = logging.handlers.TimedRotatingFileHandler(
                log_file, when="midnight", interval=1, backupCount=log_max_files, encoding="utf-8"
            )
        else:
            # Size-based rotation
...
def log_api_request(
    logger: logging.Logger, method: str, path: str, user_id: Optional[str] = None, **extra: Any
) -> None:
    """
    Log an API request.
...
def log_task_event(
    logger: logging.Logger, event: str, task_id: str, agent_id: Optional[str] = None, **extra: Any
) -> None:
    """
...
def log_agent_event(logger: logging.Logger, event: str, agent_id: str, **extra: Any) -> None:
    """
    ...
    logger.info(
        ...
        extra={"event_type": "agent_event", "agent_event": event, "agent_id": agent_id, **extra},
    )
...
def log_error(logger: logging.Logger, error: Exception, context: str = "", **extra: Any) -> None:
    """
