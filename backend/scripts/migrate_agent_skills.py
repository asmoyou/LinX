...
class AgentSkillMigration:
    ...
    def find_inline_agent_skills(self) -> List[Skill]:
        ...
        with get_db_session() as session:
            ...
            skills = (
                session.query(Skill)
                .filter(Skill.skill_type == "agent_skill", Skill.storage_type != "minio")
                .all()
            )
            ...
            try:
                skills_without_md = (
                    session.query(Skill)
                    .filter(Skill.skill_type == "agent_skill", Skill.skill_md_content.is_(None))
                    .all()
                )
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Migrate agent_skill entries to langchain_tool")
    parser.add_argument(
    ...
    try:
        migration = AgentSkillMigration(dry_run=args.dry_run, backup=not args.no_backup)
