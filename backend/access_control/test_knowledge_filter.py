...
def test_admin_can_access_all_knowledge(admin_user):
    ...
    # Test team knowledge
    assert can_access_knowledge_item(
        admin_user, Action.READ, owner_id, KnowledgeAccessLevel.TEAM
    )
    
    # Test public knowledge
    assert can_access_knowledge_item(
        admin_user, Action.READ, owner_id, KnowledgeAccessLevel.PUBLIC
    )
...
def test_manager_can_access_all_knowledge(manager_user):
    ...
    assert can_access_knowledge_item(
        manager_user, Action.READ, owner_id, KnowledgeAccessLevel.TEAM
    )
    assert can_access_knowledge_item(
...
def test_user_cannot_access_team_knowledge_with_different_department(
    ...
    owner_id = str(uuid4())
    resource_attrs = {
        "department": "hr",  # Different department
        "classification": "internal"
    }
...
def test_milvus_filter_user_includes_team_with_department(
    engineering_user, engineering_attributes
):
...
def test_filter_results_team_knowledge_with_department(
    engineering_user, engineering_attributes
):
...
def test_user_can_create_new_knowledge(regular_user):
    ...
    assert check_knowledge_write_permission(
        regular_user,
        knowledge_id=None,  # New knowledge
        owner_user_id=None
    )
...
def test_viewer_cannot_write_knowledge(viewer_user):
    ...
    assert not check_knowledge_write_permission(
        viewer_user,
        knowledge_id="k1",
        owner_user_id=viewer_user.user_id
    )
