...
def validate_username(username: str) -> None:
    ...
    if len(username) > MAX_USERNAME_LENGTH:
        raise ValidationError(f"Username must be at most {MAX_USERNAME_LENGTH} characters long")
...
def validate_resource_quotas(quotas: Optional[Dict[str, int]]) -> Dict[str, int]:
    ...
    for key, value in quotas.items():
        if key not in valid_quota_keys:
            raise ValidationError(
                f"Invalid quota key '{key}'. " f"Valid keys: {', '.join(valid_quota_keys)}"
            )
...
def create_resource_quota(
    session: Session, user_id: uuid.UUID, quotas: Dict[str, int]
) -> ResourceQuota:
...
def register_user(
    session: Session, request: RegistrationRequest, is_admin: bool = False
    ...
    try:
        ...
        raise RegistrationError("Registration failed due to database constraint violation") from e
