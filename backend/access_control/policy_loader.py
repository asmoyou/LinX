...
class PolicyLoader:
    ...
    def get_policy(self, policy_id: str) -> Optional[ABACPolicy]:
        ...
        try:
            with get_db_session() as session:
                policy_model = session.query(ABACPolicyModel).filter_by(
                    policy_id=policy_id
                ).first()
    ...
    def list_policies(
        ...
        try:
            with get_db_session() as session:
                ...
                policies = [
                    self._model_to_policy(model) for model in policy_models
                ]
            ...
            with get_db_session() as session:
                ...
                if policy_model is None:
                    raise PolicyNotFoundError(
                        f"Policy {policy.policy_id} not found"
                    )
    ...
    def delete_policy(self, policy_id: str) -> bool:
        ...
        try:
            with get_db_session() as session:
                ...
                logger.info(
                    f"Deleted ABAC policy: {policy_model.name}",
    ...
    def _serialize_conditions(self, condition_group: ConditionGroup) -> Dict[str, Any]:
        ...
        return {
            ...
            "conditions": [
                self._serialize_condition(cond) for cond in condition_group.conditions
            ]
        }
    ...
    def _serialize_condition(self, condition) -> Dict[str, Any]:
        ...
        if isinstance(condition, Condition):
            ...
            return {
                ...
                "operator": condition.operator.value,
                "conditions": [
                    self._serialize_condition(c) for c in condition.conditions
                ]
            }
        else:
    ...
    def _deserialize_conditions(self, data: Dict[str, Any]) -> ConditionGroup:
        ...
        conditions = [
            self._deserialize_condition(cond_data)
    ...
    def _deserialize_condition(self, data: Dict[str, Any]):
        ...
        if cond_type == "condition":
            ...
            operator = LogicalOperator(data["operator"])
            conditions = [
                self._deserialize_condition(c) for c in data["conditions"]
            ]
            return ConditionGroup(operator=operator, conditions=conditions)
        else:
