...
def verify_agent_ownership(
    session: Session, agent_id: str, current_user: CurrentUser, action: Action = Action.READ
) -> bool:
...
def get_user_agents(session: Session, current_user: CurrentUser) -> list:
...
def can_create_agent(session: Session, current_user: CurrentUser) -> bool:
    ...
    quota = session.query(ResourceQuota).filter_by(user_id=UUID(current_user.user_id)).first()
    ...
    logger.debug(
        f"User {current_user.user_id} can create agent", extra={"user_id": current_user.user_id}
    )
    return True


def can_update_agent(session: Session, agent_id: str, current_user: CurrentUser) -> bool:
...
def can_delete_agent(session: Session, agent_id: str, current_user: CurrentUser) -> bool:
...
def can_control_agent(session: Session, agent_id: str, current_user: CurrentUser) -> bool:
...
def get_agent_owner_id(session: Session, agent_id: str) -> Optional[str]:
