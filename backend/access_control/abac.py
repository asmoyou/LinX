...
class Condition:
    ...
    def evaluate(self, context: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.warning(f"Error evaluating condition {self.attribute} {self.operator} {self.value}: {e}")
            return False
...
def create_clearance_level_policy(
    ...
    return ABACPolicy(
        ...
        conditions=ConditionGroup(
            operator=LogicalOperator.AND,
            conditions=[
                Condition("user.clearance_level", ConditionOperator.GREATER_THAN_OR_EQUAL, required_clearance),
                Condition("resource.required_clearance", ConditionOperator.LESS_THAN_OR_EQUAL, required_clearance)
            ]
        ),
        priority=priority
    )
    ...
    return ABACPolicy(
        ...
        conditions=ConditionGroup(
            ...
            conditions=[
                ...
                Condition("environment.time.hour", ConditionOperator.GREATER_THAN_OR_EQUAL, end_hour)
            ]
        ),
        priority=priority
    )
