...
class TestPolicyLoader:
    ...
    def test_roundtrip_serialization(self, policy_loader, complex_policy):
        ...
        assert len(restored_policy.conditions.conditions) == len(
            complex_policy.conditions.conditions
        )
    ...
    def test_all_logical_operators_serialize(self, policy_loader):
        ...
        for op in operators:
            condition_group = ConditionGroup(
                operator=op, conditions=[Condition("test.attr", ConditionOperator.EQUALS, "value")]
            )
