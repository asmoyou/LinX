...
def _logging_streams_available() -> bool:
    """Return False when logger handlers are already shutting down/closed."""
    if sys.is_finalizing():
        return False
    for current_logger in (logging.getLogger(), logger):
...
class ProcessManager:
    ...
    def start_process(self, config) -> str:
        ...
        with self.lock:
            # Check process limit
            active_count = sum(
                1 for s in self.sessions.values() if s.status == ProcessStatus.RUNNING
            )
            if active_count >= self.max_processes:
                raise RuntimeError(f"Maximum processes ({self.max_processes}) exceeded")
        ...
        try:
            ...
            self.logger.info(
                f"Background process started", extra={"session_id": session_id, "pid": process.pid}
            )
    ...
    def _start_output_capture(
        self, session_id: str, process: subprocess.Popen, buffer: RingBuffer
    ) -> None:
        ...
        stdout_thread = threading.Thread(
            target=capture_stream, args=(process.stdout, ""), daemon=True
        )
        stdout_thread.start()

        # Start stderr capture thread
        stderr_thread = threading.Thread(
            target=capture_stream, args=(process.stderr, "[STDERR] "), daemon=True
        )
        stderr_thread.start()
    ...
    def kill(self, session_id: str) -> bool:
        ...
        try:
            self.logger.info(f"Killing process", extra={"session_id": session_id})
        ...
        with self.lock:
            ...
            for session_id, session in self.sessions.items():
                ...
                if (
                    ...
                    and (now - session.completed_at).total_seconds() > 86400
                ):
            ...
            for session_id in to_remove:
                ...
                self.logger.info(f"Cleaned up old session", extra={"session_id": session_id})
