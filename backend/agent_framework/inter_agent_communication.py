...
class InterAgentCommunicator:
    ...
    def _handle_incoming_message(self, message: Message) -> None:
        ...
        # Authorization check
        if self.authorizer and not self.authorizer.authorize_receive(
            message, self.agent_id
        ):
            logger.warning(
    ...
    def _handle_request(self, message: Message) -> None:
        ...
        if handler:
            try:
                ...
                asyncio.create_task(
                    self.send_response(message, response_payload)
                )
                ...
                asyncio.create_task(
                    self.send_response(message, error_payload)
                )
        else:
            logger.warning(
