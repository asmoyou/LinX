...
class TestAuthenticationSecurity:
    ...
    def test_brute_force_protection(self, api_client):
        ...
        for i in range(10):
            response = api_client.post(
                "/api/v1/auth/login",
                json={"username": user_data["username"], "password": f"WrongPass{i}"},
            )
...
class TestAuthorizationSecurity:
    ...
    def test_resource_ownership_validation(self, api_client):
        ...
        agent_response = api_client.post(
            ...
            json={"name": "User1 Agent", "agent_type": "assistant", "capabilities": ["test"]},
        )
        ...
        if agent_response.status_code == 201:
            ...
            response = api_client.get(
                f"/api/v1/agents/{agent_id}", headers={"Authorization": f"Bearer {token2}"}
            )
    ...
    def test_privilege_escalation_prevention(self, api_client):
        ...
        response = api_client.put(
            "/api/v1/users/me", headers={"Authorization": f"Bearer {token}"}, json={"role": "admin"}
        )
