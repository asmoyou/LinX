...
class TestConcurrentAgents:
        ...
        def create_agent(agent_num):
            ...
            try:
                response = authenticated_client.post(
                    "/api/v1/agents",
                    json={"name": f"Perf Agent {agent_num}", "template_id": template_id},
                )
                ...
                return {
                    ...
                    "agent_id": (
                        response.json().get("agent_id") if response.status_code == 201 else None
                    ),
    ...
    def test_100_agents_executing_tasks_concurrently(self, authenticated_client):
        ...
        for i in range(num_agents):
            response = authenticated_client.post(
                "/api/v1/agents", json={"name": f"Exec Agent {i}", "template_id": template_id}
            )
        ...
        def assign_and_execute_task(agent_id, task_num):
            ...
            try:
                ...
                return {
                    ...
                    "success": response.status_code == 201,
                    "task_id": (
                        response.json().get("task_id") if response.status_code == 201 else None
                    ),
                    "latency": time.time() - start_time,
                }
            except Exception as e:
    ...
    def test_agent_resource_utilization(self, authenticated_client):
        ...
        for i in range(num_agents):
            response = authenticated_client.post(
                "/api/v1/agents", json={"name": f"Resource Agent {i}", "template_id": template_id}
            )
            if response.status_code == 201:
                agent_ids.append(response.json()["agent_id"])
    ...
    def test_agent_lifecycle_performance(self, authenticated_client):
        ...
        for cycle in range(num_cycles):
            ...
            create_response = authenticated_client.post(
                "/api/v1/agents",
                json={"name": f"Lifecycle Agent {cycle}", "template_id": template_id},
            )
    ...
    def test_agent_communication_overhead(self, authenticated_client):
        ...
        for i in range(num_agents):
            response = authenticated_client.post(
                "/api/v1/agents", json={"name": f"Comm Agent {i}", "template_id": template_id}
            )
            if response.status_code == 201:
                agent_ids.append(response.json()["agent_id"])
