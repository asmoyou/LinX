...
class TestHorizontalScaling:
    ...
    def test_scaling_up_performance(self, api_client, authenticated_token):
        ...
        if len(results_by_config) >= 2:
            ...
            assert scaling_efficiency >= 0.7, f"Scaling efficiency {scaling_efficiency*100:.1f}% too low"
    ...
    def test_cache_consistency_across_instances(self, api_client, authenticated_token):
        ...
        update_response = api_client.put(
            "/api/v1/users/me",
            headers=headers,
            json={"full_name": f"Updated User {uuid4()}"}
        )
        
        updated_name = update_response.json().get("full_name") if update_response.status_code == 200 else None
