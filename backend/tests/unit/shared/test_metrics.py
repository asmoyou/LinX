...
def test_api_metrics_increment():
    ...
    new_value = api_requests_total.labels(method="GET", endpoint="/test", status="200")._value.get()
...
def test_llm_metrics_increment():
    ...
    llm_requests_total.labels(provider="ollama", model="llama2").inc()
...
def test_track_time_decorator_sync():
    ...
    test_metric = Histogram("test_duration_seconds", "Test duration", registry=registry)
...
async def test_track_time_decorator_async():
    ...
    test_metric = Histogram("test_async_duration_seconds", "Test async duration", registry=registry)
...
def test_histogram_observe():
    ...
    api_request_duration_seconds.labels(method="GET", endpoint="/test").observe(0.5)

    api_request_duration_seconds.labels(method="GET", endpoint="/test").observe(1.5)
...
def test_counter_increment_by_value():
    ...
    initial = llm_tokens_used_total.labels(
        provider="ollama", model="llama2", type="input"
    )._value.get()

    llm_tokens_used_total.labels(provider="ollama", model="llama2", type="input").inc(100)
