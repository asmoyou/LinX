...
def test_user_can_access_own_agent_memory(regular_user):
    ...
    assert can_access_agent_memory(regular_user, agent_id, regular_user.user_id)
...
def test_user_cannot_access_others_agent_memory(regular_user):
    ...
    assert not can_access_agent_memory(regular_user, agent_id, other_user_id)
...
def test_viewer_cannot_access_agent_memory(viewer_user):
    ...
    assert not can_access_agent_memory(viewer_user, agent_id, agent_owner_id)
...
def test_invalid_role_denies_agent_memory_access():
    ...
    assert not can_access_agent_memory(invalid_user, agent_id, agent_owner_id)
...
def test_admin_can_access_all_company_memory(admin_user):
    ...
    assert can_access_company_memory(admin_user, MemoryType.USER_CONTEXT, str(uuid4()))
    assert can_access_company_memory(admin_user, "task_context")
    assert can_access_company_memory(admin_user, "general")


def test_manager_can_access_all_company_memory(manager_user):
    """Test that manager can access all company memory."""
    assert can_access_company_memory(manager_user, MemoryType.USER_CONTEXT, str(uuid4()))
    assert can_access_company_memory(manager_user, "task_context")


def test_user_can_access_own_user_context(regular_user):
    """Test that user can access their own user context."""
    assert can_access_company_memory(regular_user, MemoryType.USER_CONTEXT, regular_user.user_id)
...
def test_abac_policy_grants_company_memory_access(mock_abac, regular_user):
    ...
    result = can_access_company_memory(
        regular_user, "confidential", user_attributes=user_attrs, resource_attributes=resource_attrs
    )
...
def test_company_memory_filter_admin_with_memory_type(admin_user):
    ...
    filter_expr = build_company_memory_filter(admin_user, memory_type="task_context")
...
def test_company_memory_filter_with_additional_filters(regular_user):
    ...
    filter_expr = build_company_memory_filter(regular_user, additional_filters=additional)
...
def test_company_memory_filter_specific_memory_type(regular_user):
    ...
    filter_expr = build_company_memory_filter(regular_user, memory_type="task_context")
...
def test_filter_agent_memory_results_admin_returns_all(admin_user):
    ...
    filtered = filter_memory_results(results, admin_user, MemoryType.AGENT_MEMORY)
...
def test_filter_agent_memory_results_user_filters_by_ownership(regular_user):
    ...
    filtered = filter_memory_results(results, regular_user, MemoryType.AGENT_MEMORY)
...
def test_filter_company_memory_results_user_context(regular_user):
    ...
    results = [
        ...
        {"memory_id": "m3", "memory_type": "general", "user_id": None, "content": "general memory"},
    ]

    filtered = filter_memory_results(results, regular_user, MemoryType.COMPANY_MEMORY)
...
def test_filter_memory_results_empty_list(regular_user):
    ...
    filtered = filter_memory_results([], regular_user, MemoryType.AGENT_MEMORY)
...
def test_admin_can_write_any_memory(admin_user):
    ...
    assert check_memory_write_permission(admin_user, MemoryType.AGENT_MEMORY)
    assert check_memory_write_permission(admin_user, MemoryType.COMPANY_MEMORY)
...
def test_user_cannot_write_others_agent_memory(regular_user):
    ...
    assert not check_memory_write_permission(
        regular_user, MemoryType.AGENT_MEMORY, agent_id=agent_id, agent_owner_id=other_user_id
    )


def test_user_can_write_company_memory(regular_user):
    """Test that user can write company memory."""
    assert check_memory_write_permission(regular_user, MemoryType.COMPANY_MEMORY)


def test_viewer_cannot_write_memory(viewer_user):
    """Test that viewer cannot write memory."""
    assert not check_memory_write_permission(viewer_user, MemoryType.AGENT_MEMORY)
    assert not check_memory_write_permission(viewer_user, MemoryType.COMPANY_MEMORY)
...
def test_admin_can_delete_any_memory(admin_user):
    ...
    assert check_memory_delete_permission(
        admin_user, MemoryType.AGENT_MEMORY, agent_owner_id=str(uuid4())
    )
    assert check_memory_delete_permission(
        admin_user, MemoryType.COMPANY_MEMORY, memory_user_id=str(uuid4())
    )
...
def test_user_can_delete_own_agent_memory(regular_user):
    ...
    assert check_memory_delete_permission(
        regular_user, MemoryType.AGENT_MEMORY, agent_owner_id=regular_user.user_id
    )
...
def test_user_cannot_delete_others_agent_memory(regular_user):
    ...
    assert not check_memory_delete_permission(
        regular_user, MemoryType.AGENT_MEMORY, agent_owner_id=other_user_id
    )
...
def test_user_can_delete_own_company_memory(regular_user):
    ...
    assert check_memory_delete_permission(
        regular_user, MemoryType.COMPANY_MEMORY, memory_user_id=regular_user.user_id
    )
...
def test_user_cannot_delete_others_company_memory(regular_user):
    ...
    assert not check_memory_delete_permission(
        regular_user, MemoryType.COMPANY_MEMORY, memory_user_id=other_user_id
    )
...
def test_viewer_cannot_delete_memory(viewer_user):
    ...
    assert not check_memory_delete_permission(
        viewer_user, MemoryType.AGENT_MEMORY, agent_owner_id=viewer_user.user_id
    )
