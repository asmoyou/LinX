...
class TestAgentCreationFromTemplate:
    ...
    def test_complete_agent_creation_flow(self, authenticated_client):
        ...
        data_analyst_template = next(
            (t for t in templates if "data" in t["name"].lower() and "analyst" in t["name"].lower()),
        ...
        assert agent_status["status"] in ["working", "idle"]  # Might be working or completed quickly
        
        # Step 9: Update agent configuration
        update_data = {
            "description": "Updated: Agent for comprehensive data analysis"
        }
