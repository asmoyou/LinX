...
class TestMultiAgentCollaboration:
    ...
    def test_complete_collaboration_workflow(self, authenticated_client_with_multiple_agents):
        ...
        if final_task["status"] == "completed":
            ...
            if "contributors" in final_task or "agents_involved" in final_task:
                contributors_key = (
                    "contributors" if "contributors" in final_task else "agents_involved"
                )
    ...
    def test_agent_handoff_between_tasks(self, authenticated_client_with_multiple_agents):
        ...
        goal_response = client.post(
            "/api/v1/tasks",
            json={"goal_text": "First collect data, then analyze it", "priority": 1},
        )
        ...
        if tree_response.status_code == 200:
            ...
            if subtasks_key in tree and len(tree[subtasks_key]) >= 2:
                ...
                agent_assignments = [
                    st.get("assigned_agent_id") for st in subtasks if "assigned_agent_id" in st
                ]
    ...
    def test_agent_communication_during_collaboration(
        self, authenticated_client_with_multiple_agents
    ):
            ...
            if task["status"] == "completed":
                ...
                if "result" in task:
                    ...
                    if isinstance(result, dict):
                        assert (
                            "aggregated" in str(result).lower() or "combined" in str(result).lower()
                        )
                break
    ...
    def test_agent_specialization_in_collaboration(self, authenticated_client_with_multiple_agents):
        ...
        goal_response = client.post(
            "/api/v1/tasks",
            json={"goal_text": "Analyze data, write code, and create documentation", "priority": 1},
        )
    ...
    def test_load_balancing_across_agents(self, authenticated_client_with_multiple_agents):
        ...
        for i in range(5):
            response = client.post(
                "/api/v1/tasks", json={"goal_text": f"Task {i}: Process some data", "priority": 1}
            )
