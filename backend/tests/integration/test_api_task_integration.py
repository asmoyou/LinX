...
def mock_task_coordinator():
    ...
    with patch('task_manager.task_coordinator.TaskCoordinator') as mock:
        coordinator = Mock()
        coordinator.submit_goal = AsyncMock(return_value={
            'task_id': str(uuid4()),
            'status': 'pending',
            'goal_text': 'Test goal'
        })
        coordinator.get_task_status = AsyncMock(return_value={
            'task_id': str(uuid4()),
            'status': 'in_progress',
            'progress': 50
        })
...
class TestAPITaskManagerIntegration:
    ...
    async def test_submit_goal_creates_task(self, mock_db_session, mock_task_coordinator):
        ...
        response = client.post(
            "/api/v1/tasks",
            json={
                "goal_text": "Analyze sales data for Q4",
                "priority": 1
            },
    ...
    @pytest.mark.asyncio
    async def test_get_task_status_retrieves_from_manager(self, mock_db_session, mock_task_coordinator):
    ...
    async def test_task_list_filters_by_user(self, mock_db_session, mock_task_coordinator):
        ...
        mock_task_coordinator.list_tasks = AsyncMock(return_value=[
