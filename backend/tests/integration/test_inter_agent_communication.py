...
def mock_communicator():
    ...
    with patch("agent_framework.inter_agent_communication.get_communicator") as mock:
        ...
        communicator.send_message = AsyncMock(
            return_value={"message_id": str(uuid4()), "status": "delivered"}
        )
        communicator.request_assistance = AsyncMock(
            return_value={"response": "Assistance provided", "from_agent": str(uuid4())}
        )
...
class TestInterAgentCommunication:
    ...
    async def test_agent_receives_and_processes_message(self, mock_message_bus):
        ...
        mock_message_bus.get_messages = AsyncMock(
            return_value=[
                {
    ...
    async def test_message_acknowledgment(self, mock_communicator):
        ...
        ack = await communicator.wait_for_acknowledgment(message_id=result["message_id"], timeout=5)
