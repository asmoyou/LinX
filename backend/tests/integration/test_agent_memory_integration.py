...
def mock_memory_interface():
    ...
    with patch('memory_system.memory_interface.get_memory_interface') as mock:
        ...
        interface.retrieve_agent_memory = AsyncMock(return_value=[
            {'content': 'Previous interaction', 'relevance': 0.95}
        ])
        interface.store_company_memory = AsyncMock(return_value=str(uuid4()))
        interface.retrieve_company_memory = AsyncMock(return_value=[
            {'content': 'Company knowledge', 'relevance': 0.90}
        ])
...
class TestAgentMemoryIntegration:
    ...
    async def test_agent_accesses_company_memory(self, mock_memory_interface):
        ...
        knowledge = await agent.retrieve_company_knowledge(
            query="Company policies on data retention",
            limit=3
        )
    ...
    @pytest.mark.asyncio
    async def test_agent_memory_includes_embeddings(self, mock_memory_interface, mock_embedding_service):
        ...
        await agent.store_memory(
            content="Important business decision made",
