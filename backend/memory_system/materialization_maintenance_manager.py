"""Scheduled maintenance manager for memory materializations."""
...
class MaterializationMaintenanceSettings:
    """Configuration for scheduled materialization maintenance."""
    ...
    def with_defaults(self) -> "MaterializationMaintenanceSettings":
        return MaterializationMaintenanceSettings(
            ...
            include_consolidation=self.include_consolidation,
...
def load_materialization_maintenance_settings(
    ...
    """Load settings from ``memory.materialization_maintenance``."""

    cfg = config or get_config()
    raw = cfg.get("memory.materialization_maintenance", {}) or {}
    ...
    settings = MaterializationMaintenanceSettings(
        ...
        limit=limit,
        include_backfill=_cfg_bool(raw.get("include_backfill"), True),
        include_consolidation=_cfg_bool(raw.get("include_consolidation"), True),
...
def run_materialization_maintenance_once(
    ...
    *,
    ...
    """Run one materialization maintenance cycle."""
    ...
    try:
        ...
        result = service.run_maintenance(
            ...
            limit=cfg.limit,
            include_backfill=cfg.include_backfill,
            include_consolidation=cfg.include_consolidation,
        )
        payload = service.to_dict(result)
        backfill = payload.get("backfill") or {}
        consolidation = payload.get("consolidation") or {}
        total_upserts = (
            int(backfill.get("user_profile_upserts") or 0)
            + int(backfill.get("agent_experience_upserts") or 0)
            + int(backfill.get("user_entry_upserts") or 0)
            + int(backfill.get("agent_entry_upserts") or 0)
        )
        total_updates = (
...
class MaterializationMaintenanceManager:
    """Periodic scheduler for materialization backfill and consolidation."""
    ...
    async def start(self) -> bool:
        ...
        logger.info(
            ...
            extra={
                ...
                "include_backfill": self.settings.include_backfill,
                "include_consolidation": self.settings.include_consolidation,
            },
        )
        return True
        ...
        async with self._run_lock:
            ...
            logger.log(
                ...
                extra={
                    ...
                    "total_upserts": result.get("total_upserts"),
