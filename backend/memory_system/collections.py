...
def create_agent_memories_schema() -> CollectionSchema:
    ...
    fields = [
        ...
        FieldSchema(
            name="timestamp", dtype=DataType.INT64, description="Unix timestamp in milliseconds"
        ),
        FieldSchema(
            name="metadata",
            dtype=DataType.JSON,
...
def create_knowledge_embeddings_schema() -> CollectionSchema:
    ...
    schema = CollectionSchema(
        fields=fields, description="Knowledge base document embeddings", enable_dynamic_field=False
    )
...
def create_collection(
    collection_name: str, schema: CollectionSchema, drop_if_exists: bool = False
) -> Collection:
    """
    ...
    try:
        ...
        # Create collection
        collection = Collection(name=collection_name, schema=schema, using=manager.connection_alias)
...
def create_index(
    ...
    if index_params is None:
        if index_type == IndexType.IVF_FLAT:
            index_params = {"nlist": milvus_config.get("nlist", 1024)}
    ...
    try:
        ...
        collection.create_index(field_name=field_name, index_params=index_config)
...
def create_partition(collection: Collection, partition_name: str) -> None:
    """
...
def initialize_all_collections(drop_if_exists: bool = False) -> Dict[str, Collection]:
    ...
    try:
        ...
        agent_memories = create_collection(
            CollectionName.AGENT_MEMORIES, agent_memories_schema, drop_if_exists=drop_if_exists
        )
        ...
        company_memories = create_collection(
            CollectionName.COMPANY_MEMORIES, company_memories_schema, drop_if_exists=drop_if_exists
        )
