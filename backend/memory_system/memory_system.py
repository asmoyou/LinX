...
class MemorySystem(MemorySystemInterface):
    ...
    def _determine_search_collections(self, query: SearchQuery) -> List[str]:
        ...
        elif query.memory_type in [MemoryType.COMPANY, MemoryType.USER_CONTEXT, MemoryType.TASK_CONTEXT]:
    ...
    def _rank_results(self, results: List[MemoryItem]) -> List[MemoryItem]:
        ...
        ranked = sorted(
            results,
            key=lambda m: m.metadata.get('_combined_score', 0.0),
            reverse=True
        )
    ...
    def classify_memory_type(
        ...
        user_keywords = [
            ...
            'my', 'i am', 'i have', 'remember that i'
        ]
        
        # Task context keywords
        task_keywords = [
            'task', 'step', 'result', 'output', 'completed', 'failed',
            'processing', 'analyzed', 'generated'
        ]
    ...
    def share_memory(
        ...
        try:
            ...
            logger.info(
                f"Shared memory {memory_id} with {len(target_user_ids)} users"
            )
            return True
