...
class MessageAuthorizer:
    ...
    def authorize_message(self, message: Message) -> tuple[bool, Optional[str]]:
        ...
        if message.task_id not in sender_perms.assigned_tasks:
            return False, (f"Agent {message.from_agent_id} not assigned to task {message.task_id}")
    ...
    def can_agent_message_agent(self, sender_id: str, recipient_id: str, task_id: str) -> bool:
        """
    ...
    def update_agent_tasks(self, agent_id: str, task_ids: Set[str]) -> None:
        ...
        if perms:
            ...
            logger.debug(f"Updated task assignments for agent {agent_id}: {task_ids}")
        else:
            logger.warning(f"Cannot update tasks for unregistered agent {agent_id}")
    ...
    def add_agent_task(self, agent_id: str, task_id: str) -> None:
        ...
        if perms:
            ...
            logger.warning(f"Cannot add task for unregistered agent {agent_id}")
    ...
    def remove_agent_task(self, agent_id: str, task_id: str) -> None:
        ...
        if perms:
            ...
            logger.warning(f"Cannot remove task for unregistered agent {agent_id}")
