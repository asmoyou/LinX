...
class SatisfactionTracker:
    ...
    def calculate_metrics(
        ...
        # Filter surveys
        filtered_surveys = [s for s in self.surveys if start_date <= s.submitted_at <= end_date]
    ...
    def _determine_trend(
        ...
        current_surveys = [s for s in self.surveys if start_date <= s.submitted_at <= end_date]

        # Filter surveys for previous period
        previous_surveys = [
            s for s in self.surveys if previous_start <= s.submitted_at <= previous_end
        ]
    ...
    def _generate_insights(
        ...
        if metrics.avg_overall_rating >= 4.0:
            insights.append(
                f"High user satisfaction with average rating of {metrics.avg_overall_rating:.1f}/5"
            )
        elif metrics.avg_overall_rating < 3.0:
            insights.append(
                f"Low user satisfaction ({metrics.avg_overall_rating:.1f}/5) requires immediate attention"
            )

        # NPS insight
        if metrics.nps_score >= 50:
            insights.append(
                f"Excellent NPS score of {metrics.nps_score:.0f} indicates strong user loyalty"
            )
        elif metrics.nps_score < 0:
            insights.append(
                f"Negative NPS score ({metrics.nps_score:.0f}) indicates user dissatisfaction"
            )
    ...
    def get_top_rated_agents(
        ...
        for survey in self.surveys:
            if survey.agent_id and start_date <= survey.submitted_at <= end_date:
