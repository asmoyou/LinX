...
class ResourceForecaster:
    ...
    def forecast_resource_usage(
        ...
        if model == ForecastModel.MOVING_AVERAGE:
            result.forecasted_values = self._moving_average_forecast(
                result.historical_values, forecast_hours
            )
        ...
        last_timestamp = (
            result.historical_timestamps[-1] if result.historical_timestamps else datetime.now()
        )
        result.forecasted_timestamps = [
            last_timestamp + timedelta(hours=i + 1) for i in range(forecast_hours)
        ]

        # Calculate confidence intervals
        result.lower_bound, result.upper_bound = self._calculate_confidence_intervals(
            result.historical_values, result.forecasted_values
        )
        ...
        result.capacity_warnings = self._generate_capacity_warnings(
            resource_type, result.forecasted_values
        )
    ...
    def _generate_capacity_warnings(
        ...
        if max_forecast > threshold_percent:
            hours_to_threshold = next(
                (i for i, v in enumerate(forecasted_values) if v > threshold_percent), None
            )
    ...
    def forecast_agent_capacity(
        ...
        return {
            ...
            "utilization_percent": (
                (current_agents / required_agents * 100) if required_agents > 0 else 0
            ),
            "recommendation": (
                "Scale up"
                if required_agents > current_agents * 1.2
