...
class AnomalyDetector:
    ...
    def _detect_performance_degradation(
        ...
        if "tasks_per_hour" in metrics and "tasks_per_hour" in baseline:
            ...
            deviation = (
                ((baseline_value - avg_recent) / baseline_value * 100) if baseline_value > 0 else 0
            )
        ...
        if "cpu_usage" in metrics and "cpu_usage" in baseline:
            ...
            if max_value > baseline_value * 2 and max_value > 80:  # 2x baseline and >80%
                alert = AnomalyAlert(
                    ...
                    expected_value=baseline_value,
                    deviation_percent=(
                        ((max_value - baseline_value) / baseline_value * 100)
                        if baseline_value > 0
                        else 0
                    ),
                    recommended_actions=[
            ...
            if len(recent_values) >= 4:
                ...
                if outliers:
                    ...
                    alert = AnomalyAlert(
                        ...
                        expected_value=baseline_value,
                        deviation_percent=(
                            ((max_outlier - baseline_value) / baseline_value * 100)
                            if baseline_value > 0
                            else 0
                        ),
                        recommended_actions=[
        ...
        return {
            ...
            "by_type": {
                anomaly_type.value: len(
                    [a for a in recent_alerts if a.anomaly_type == anomaly_type]
                )
