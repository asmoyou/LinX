...
class CostTracker:
        ...
        return {
            ...
            "breakdown": {
                "llm": {
                    ...
                    "percent": (metrics.llm_cost_usd / metrics.total_cost_usd * 100) if metrics.total_cost_usd > 0 else 0,
                    "api_calls": metrics.llm_api_calls,
                    "tokens_used": metrics.llm_tokens_used,
                },
                "compute": {
                    "cost_usd": metrics.compute_cost_usd,
                    "percent": (metrics.compute_cost_usd / metrics.total_cost_usd * 100) if metrics.total_cost_usd > 0 else 0,
                    "cpu_hours": metrics.cpu_hours,
                    "memory_gb_hours": metrics.memory_gb_hours,
                },
                "storage": {
                    "cost_usd": metrics.storage_cost_usd,
                    "percent": (metrics.storage_cost_usd / metrics.total_cost_usd * 100) if metrics.total_cost_usd > 0 else 0,
                    "storage_gb": metrics.storage_gb,
                },
                "network": {
                    "cost_usd": metrics.network_cost_usd,
                    "percent": (metrics.network_cost_usd / metrics.total_cost_usd * 100) if metrics.total_cost_usd > 0 else 0,
    ...
    def compare_costs(
        ...
        cost_change_percent = (cost_change / previous_metrics.total_cost_usd * 100) if previous_metrics.total_cost_usd > 0 else 0
...
class CostOptimizer:
    ...
    def get_optimization_summary(
        ...
        return {
            ...
            "savings_percent": (total_potential_savings / current_cost * 100) if current_cost > 0 else 0,
            ...
            "by_category": {
                ...
                "compute": sum(r.potential_savings_usd for r in recommendations if r.category == "compute"),
                "storage": sum(r.potential_savings_usd for r in recommendations if r.category == "storage"),
                "network": sum(r.potential_savings_usd for r in recommendations if r.category == "network"),
            },
            "top_recommendations": [
                {
